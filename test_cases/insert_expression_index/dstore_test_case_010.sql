-- INSERT INTO `{{TEST_TABLE_NAME}}` SELECT * FROM `{{TEST_TABLE_NAME}}_src` WHERE `bigint_col` IS NOT NULL;
-- 测试表达式索引下从 C 插入1万行WHERE bigint_col IS NOT NULL的执行情况
...
ALTER TABLE `{{TEST_TABLE_NAME}}` ADD INDEX `idx_insert_expr` ((ABS(`bigint_col`)));
