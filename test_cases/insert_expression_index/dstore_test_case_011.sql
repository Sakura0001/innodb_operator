-- INSERT INTO `{{TEST_TABLE_NAME}}` SELECT * FROM `{{TEST_TABLE_NAME}}_src` WHERE `bigint_col` % 1000 = 0;
-- 测试表达式索引下从 C 插入1万行触发 ABS 边界 bigint 模1000的执行情况
