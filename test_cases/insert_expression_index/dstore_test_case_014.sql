-- INSERT INTO `{{TEST_TABLE_NAME}}` SELECT * FROM (SELECT * FROM `{{TEST_TABLE_NAME}}_src` WHERE `id_col` % 3 = 0) t;
-- 测试表达式索引下从 C 插入1万行派生表插入的执行情况
