-- INSERT INTO `{{TEST_TABLE_NAME}}` SELECT * FROM `{{TEST_TABLE_NAME}}_src` WHERE `int_col` BETWEEN -100000 AND 100000;
-- 测试唯一索引下从 C 插入1万行非唯一列 int_col 过滤的执行情况
...
ALTER TABLE `{{TEST_TABLE_NAME}}` ADD UNIQUE INDEX `idx_insert_uq_vc` (`varchar_col`);
