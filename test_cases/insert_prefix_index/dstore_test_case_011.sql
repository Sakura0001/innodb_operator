-- INSERT INTO `{{TEST_TABLE_NAME}}` SELECT * FROM `{{TEST_TABLE_NAME}}_src` WHERE `date_col` IS NOT NULL;
-- 测试前缀索引下从 C 插入1万行非索引列过滤 date_col的执行情况
...
ALTER TABLE `{{TEST_TABLE_NAME}}` ADD INDEX `idx_insert_prefix` (`varchar_col`(32));
