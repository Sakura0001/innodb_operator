-- INSERT INTO `{TEST_TABLE_NAME}` SELECT * FROM `{TEST_TABLE_NAME}_src` WHERE `varchar_col` IS NOT NULL;
-- 测试逆序索引 varchar_col DESC 下从 C 插入1万行WHERE varchar_col IS NOT NULL的执行情况
