-- INSERT INTO `{{TEST_TABLE_NAME}}` SELECT * FROM `{{TEST_TABLE_NAME}}_src` ORDER BY `year_col` DESC;
-- 测试组合索引下从 C 插入1万行ORDER BY 索引首列 year_col的执行情况
