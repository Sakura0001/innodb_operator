-- INSERT INTO `{{TEST_TABLE_NAME}}` SELECT * FROM `{{TEST_TABLE_NAME}}_src` WHERE `varchar_col` LIKE 'c_%' OR `varchar_col` IS NOT NULL;
-- 测试组合索引下从 C 插入1万行部分索引列过滤 varchar的执行情况
