-- INSERT INTO `{{TEST_TABLE_NAME}}` SELECT * FROM `{{TEST_TABLE_NAME}}_src` WHERE `year_col` IS NOT NULL;
-- 测试组合索引下从 C 插入1万行WHERE year_col IS NOT NULL的执行情况
